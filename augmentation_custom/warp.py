...
class Stretch:
    ...
    def __call__(self, img, mag=-1, prob=1.):
        ...
        # b = [.2, .3, .4]
        b = [random.uniform(0.1,0.4), random.uniform(0.1,0.4), random.uniform(0.1,0.4)]
...
class Distort:
    ...
    def __call__(self, img, mag=-1, prob=1.):
        ...
        b = [random.uniform(0,0.1), random.uniform(0,0.1), random.uniform(0,0.1)]
        if mag < 0 or mag >= len(b):
            index = len(b) - 1
        else:
            index = mag
...
if __name__ == "__main__":
    ...
    for i in range(0,100):
        augmented_img_1 = stretch(img, margin, prob)
        augmented_img_1.save("./result/aug_img{}.png".format(i))
    print(time.time()-t1)
    """
    using Stretch, Distort
    """